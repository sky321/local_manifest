...
for i in ${bin_list}
do
  cp -v /usr/local/src/oxygenos-4.0.2/system/bin/${i} ${PREBUILT_PATH}bin/${i}
done
...
firmware/mbn_ota/mcfg_sw/mbn_ota.txt
...
firmware/mbn_ota/mcfg_sw_NoCDMA/mbn_ota.txt
firmware/mbn_ota/mcfg_sw_NoCDMA/mcfg_sw_Airtel-Commercial.mbn
...
firmware/mbn_ota/mcfg_sw_NoCDMA/mcfg_sw_EE-Commercial.mbn
firmware/mbn_ota/mcfg_sw_NoCDMA/mcfg_sw_Elisa-VoLTE.mbn
firmware/mbn_ota/mcfg_sw_NoCDMA/mcfg_sw_MTNL_BSNL.mbn
...
firmware/mbn_ota/mcfg_sw_NoCDMA/mcfg_sw_UK-VoLTE.mbn
...
firmware/tp/fw_synaptics_15811.img
...
libnfc-nxp_lingnantong.conf
libnfc-nxp_shenzhentong.conf
libnfc-nxp_yangchengtong.conf
lowi.conf
mmi/fail.png
mmi/fonts.ttf
...
nfc_card_config.conf
nfcee_access.xml
nfcscc_access.xml
nfcse_access.xml
oneplus_comp_whitelist.txt
oneplus_ftm_test.wav
permissions/android.hardware.fingerprint.xml
permissions/android.hardware.nfc.hcef.xml
permissions/android.hardware.nfc.hce.xml
permissions/android.hardware.nfc.xml
permissions/android.software.verified_boot.xml
permissions/btmultisim.xml
permissions/cneapiclient.xml
permissions/com.android.nfc_extras.xml
permissions/com.google.android.maps.xml
permissions/com.google.android.media.effects.xml
permissions/com.google.widevine.software.drm.xml
permissions/com.nxp.mifare.xml
permissions/com.oem.feature.xml
permissions/com.oneplus.software.xml
...
permissions/lpa.xml
permissions/oneplus_sdk_utils.xml
permissions/oneplus_sdk_wrapper.xml
permissions/org.simalliance.openmobileapi.xml
...
for i in ${etc_list}
do
  cp -v /usr/local/src/oxygenos-4.0.2/system/etc/${i} ${PREBUILT_PATH}etc/${i}
done
...
libop_shape.so
libop_stitching.so
libop_structure_light.so
libop_superpixel.so
libop_superres.so
libop_surface_matching.so
libop_tiff.so
libop_ts.so
libop_videoio.so
libop_video.so
...
modules/spidev.ko
qpd_dspcl_v2.bin
rfsa/adsp/libmorpho_image_stabilizer4_hvx_skel.so
rfsa/adsp/libmorpho_video_denoiser_hvx_skel.so
soundfx/libqcompostprocbundle.so
soundfx/libqcomvisualizer.so
soundfx/libqcomvoiceprocessing.so
TC_Bayer_Converter_v6_core_opt.bin
"

for i in ${lib_list}
do
  cp -v /usr/local/src/oxygenos-4.0.2/system/lib/${i} ${PREBUILT_PATH}lib/${i}
done
...
for i in ${lib64_list}
do
  cp -v /usr/local/src/oxygenos-4.0.2/system/lib64/${i} ${PREBUILT_PATH}lib64/${i}
done
...
srec/en-US/APP_NAME.fst
srec/en-US/APP_NAME.syms
srec/en-US/c_fst
srec/en-US/class_normalizer.mfar
srec/en-US/CLG.prewalk.fst
srec/en-US/commands.abnf
srec/en-US/compile_grammar.config
srec/en-US/config.pumpkin
srec/en-US/confirmation_bias.fst
srec/en-US/CONTACT_NAME.fst
srec/en-US/CONTACT_NAME.syms
srec/en-US/contacts.abnf
srec/en-US/contacts_bias.fst
srec/en-US/contacts_disambig.fst
srec/en-US/dict
srec/en-US/dictation.config
srec/en-US/dnn
srec/en-US/endpointer_dictation.config
srec/en-US/endpointer_model
srec/en-US/endpointer_model.mmap
srec/en-US/endpointer_voicesearch.config
srec/en-US/ep_portable_mean_stddev
srec/en-US/ep_portable_model.mmap
srec/en-US/g2p.data
srec/en-US/g2p_fst
srec/en-US/grammar.config
srec/en-US/graphemes.syms
srec/en-US/hmmlist
srec/en-US/hmm_symbols
srec/en-US/input_mean_std_dev
srec/en-US/lexicon.U.fst
srec/en-US/lstm_model.uint8.data
srec/en-US/magic_mic.config
srec/en-US/metadata
srec/en-US/monastery_config.pumpkin
srec/en-US/normalizer.mfar
srec/en-US/norm_fst
srec/en-US/offensive_word_normalizer.mfar
srec/en-US/offline_action_data.pb
srec/en-US/phonelist
srec/en-US/phonelist.syms
srec/en-US/phonemes.syms
srec/en-US/portable_lstm
srec/en-US/portable_meanstddev
srec/en-US/pumpkin.mmap
srec/en-US/rescoring.fst.louds
srec/en-US/semantics.pumpkin
srec/en-US/SONG_NAME.fst
srec/en-US/SONG_NAME.syms
srec/en-US/voice_actions_compiler.config
srec/en-US/voice_actions.config
srec/en-US/word_classifier
srec/en-US/wordlist.syms
"

for i in ${usr_list}
do
  cp -v /usr/local/src/oxygenos-4.0.2/system/usr/${i} ${PREBUILT_PATH}usr/${i}
done
...
do
  cp -v /usr/local/src/oxygenos-4.0.2/system/vendor/${i} ${PREBUILT_PATH}vendor/${i}
done
...
do
  cp -v /usr/local/src/oxygenos-4.0.2/system/framework/${i} ${PREBUILT_PATH}framework/${i}
done
...
for i in ${sbin_list}
do
  cp -v /usr/local/src/oxygenos-4.0.2/boot/sbin/${i} ${PREBUILT_PATH}sbin/${i}
done
